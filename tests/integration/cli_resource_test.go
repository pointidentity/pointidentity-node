...
var _ = Describe("pointidentity cli - positive resource", func() {
	var tmpDir string
	...
	It("can create diddoc, create resource, query it, query all resource versions of the same resource name, query resource collection", func() {
		...
		did := "did:pointid:" + network.DidNamespace + ":" + collectionID
		keyId := did + "#key1"
		...
		secondDid := "did:pointid:" + network.DidNamespace + ":" + secondCollectionId
