...
###############################################################
LAST_N_RELEASES = 5
DEFAULT_POINTIDENTITY_HOME_DIR = "/home/pointidentity"
DEFAULT_INSTALL_PATH = "/usr/bin"
DEFAULT_POINTIDENTITY_USER = "pointidentity"
DEFAULT_BINARY_NAME = "pointidentity-noded"
DEFAULT_COSMOVISOR_BINARY_NAME = "cosmovisor"
MAINNET_CHAIN_ID = "pointidentity-mainnet-1"
TESTNET_CHAIN_ID = "pointidentity-testnet-1"
PRINT_PREFIX = "********* "
...
###############################################################
STANDALONE_SERVICE_TEMPLATE = f"https://raw.githubusercontent.com/pointidentity/pointidentity-node/{DEFAULT_DEBUG_BRANCH}/installer/templates/pointidentity-noded.service"
COSMOVISOR_SERVICE_TEMPLATE = f"https://raw.githubusercontent.com/pointidentity/pointidentity-node/{DEFAULT_DEBUG_BRANCH}/installer/templates/pointidentity-cosmovisor.service"
LOGROTATE_TEMPLATE = f"https://raw.githubusercontent.com/pointidentity/pointidentity-node/{DEFAULT_DEBUG_BRANCH}/installer/templates/logrotate.conf"
RSYSLOG_TEMPLATE = f"https://raw.githubusercontent.com/pointidentity/pointidentity-node/{DEFAULT_DEBUG_BRANCH}/installer/templates/rsyslog.conf"
DEFAULT_STANDALONE_SERVICE_NAME = 'pointidentity-noded'
DEFAULT_COSMOVISOR_SERVICE_NAME = 'pointidentity-cosmovisor'
...
DEFAULT_LOGROTATE_FILE = "/etc/logrotate.d/pointidentity-node"
DEFAULT_RSYSLOG_FILE = "/etc/rsyslog.d/pointidentity-node.conf"
DEFAULT_LOGIN_SHELL_ENV_FILE_PATH = "/etc/profile.d/pointidentity-node.sh"
BASH_PROFILE_TEMPLATE = f"https://raw.githubusercontent.com/pointidentity/pointidentity-node/{DEFAULT_DEBUG_BRANCH}/installer/templates/bash-profile.txt"


###############################################################
###     		Network configuration files    				###
###############################################################
GENESIS_FILE = "https://raw.githubusercontent.com/pointidentity/pointidentity-node/%s/networks/{}/genesis.json" % (
    DEFAULT_DEBUG_BRANCH)
SEEDS_FILE = "https://raw.githubusercontent.com/pointidentity/pointidentity-node/%s/networks/{}/seeds.txt" % (
    DEFAULT_DEBUG_BRANCH)
...
###     				Node snapshots      				###
###############################################################
TESTNET_SNAPSHOT = "https://snapshots-cdn.pointidentity.net/testnet/{}/pointidentity-testnet-1_{}.tar.lz4"
MAINNET_SNAPSHOT = "https://snapshots-cdn.pointidentity.net/mainnet/{}/pointidentity-mainnet-1_{}.tar.lz4"
MAX_SNAPSHOT_DAYS = 7
...
###############################################################
DEFAULT_RPC_PORT = "26647"
DEFAULT_P2P_PORT = "26646"
POINTIDENTITY_NODED_HOME = "/home/pointidentity/.node"
POINTIDENTITY_NODED_NODE = "tcp://localhost:26647"
POINTIDENTITY_NODED_MONIKER = platform.node()
POINTIDENTITY_NODED_CHAIN_ID = MAINNET_CHAIN_ID
POINTIDENTITY_NODED_MINIMUM_GAS_PRICES = "50upoint"
POINTIDENTITY_NODED_LOG_LEVEL = "error"
POINTIDENTITY_NODED_LOG_FORMAT = "json"
...
###############################################################
###         Release class: Get pointidentity-node releases   	    ###
###############################################################
class Release:
    ...
    def get_release_url(self):
        ...
        try:
            ...
            for _url_item in self.assets:
                ...
                if os.path.basename(_url) == f"pointidentity-noded-{version_without_v_prefix}-{os_name}-{os_arch}.tar.gz":
            ...
            logging.exception(f"Failed to get pointidentity-node binaries from GitHub. Reason: {e}")
...
class Installer():
    ...
    @property
    def pointidentity_home_dir(self):
        # Root directory for pointidentity-noded
        # Default: /home/pointidentity
        return self.interviewer.home_dir

    @property
    def pointidentity_backup_dir(self):
        # Root directory for pointidentity-noded
        # Default: /home/pointidentity/backup
        return os.path.join(self.pointidentity_home_dir, "backup")

    @property
    def pointidentity_root_dir(self):
        # Root directory for pointidentity-noded
        # Default: /home/pointidentity/.node
        return os.path.join(self.pointidentity_home_dir, ".node")

    @property
    def pointidentity_config_dir(self):
        # pointidentity-noded config directory
        # Default: /home/pointidentity/.node/config
        return os.path.join(self.pointidentity_root_dir, "config")

    @property
    def pointidentity_data_dir(self):
        # pointidentity-noded data directory
        # Default: /home/pointidentity/.node/data
        return os.path.join(self.pointidentity_root_dir, "data")

    @property
    def pointidentity_log_dir(self):
        # pointidentity-noded log directory
        # Default: /home/pointidentity/.node/log
        return os.path.join(self.pointidentity_root_dir, "log")

    @property
    def pointidentity_user_bashrc_path(self):
        # Path where .bashrc file for pointidentity user will be created
        # Default: /home/pointidentity/.bashrc
        return os.path.join(self.pointidentity_home_dir, ".bashrc")

    @property
    def pointidentity_user_bash_profile_path(self):
        # Path where .bash_profile file for pointidentity user will be created
        # Default: /home/pointidentity/.bash_profile
        return os.path.join(self.pointidentity_home_dir, ".bash_profile")

    @property
    def cosmovisor_root_dir(self):
        # cosmovisor root directory
        # Default: /home/pointidentity/.node/cosmovisor
        return os.path.join(self.pointidentity_root_dir, "cosmovisor")
    ...
    def standalone_node_binary_path(self):
        # Path where pointidentity-noded binary will be installed
        # Default: /usr/bin/pointidentity-noded
    ...
    def temporary_node_binary_path(self):
        # Temporary path for pointidentity-node binary just after it's downloaded
        # This is NOT the final install path
        return os.path.join(os.path.realpath(os.path.curdir), DEFAULT_BINARY_NAME)

    @property
    def cosmovisor_current_bin_path(self):
        # pointidentity-noded binary path if installed with cosmovisor
        # Default: /home/pointidentity/.node/cosmovisor/current/bin/pointidentity-noded
    ...
    def cosmovisor_service_cfg(self):
        # Modify pointidentity-cosmovisor.service template file to replace values for environment variables
        ...
        try:
            ...
            if is_valid_url(COSMOVISOR_SERVICE_TEMPLATE):
                with request.urlopen(COSMOVISOR_SERVICE_TEMPLATE) as response:
                    ...
                    s = re.sub(
                        r'({POINTIDENTITY_ROOT_DIR}|{DEFAULT_BINARY_NAME}|{COSMOVISOR_DAEMON_ALLOW_DOWNLOAD_BINARIES}|{COSMOVISOR_DAEMON_RESTART_AFTER_UPGRADE}|{DEFAULT_DAEMON_POLL_INTERVAL}|{DEFAULT_UNSAFE_SKIP_BACKUP}|{DEFAULT_DAEMON_RESTART_DELAY})',
                        lambda m: {'{POINTIDENTITY_ROOT_DIR}': self.pointidentity_root_dir,
        ...
        try:
            ...
            if is_valid_url(RSYSLOG_TEMPLATE):
                with request.urlopen(RSYSLOG_TEMPLATE) as response:
                    # Replace the values for environment variables in the template file
                    s = re.sub(
                        r'({BINARY_FOR_LOGGING}|{POINTIDENTITY_LOG_DIR})',
                        lambda m: {'{BINARY_FOR_LOGGING}': binary_name,
                                    '{POINTIDENTITY_LOG_DIR}': self.pointidentity_log_dir}[m.group()],
                        response.read().decode("utf-8").strip()
                    )
    ...
    def logrotate_cfg(self):
        ...
        # Logrotate is used to rotate the log files of the pointidentity-node every day, and keep a maximum of 7 days of logs.
        try:
            # Fetch the template file from GitHub
            if is_valid_url(LOGROTATE_TEMPLATE):
                with request.urlopen(LOGROTATE_TEMPLATE) as response:
                    # Replace the values for environment variables in the template file
                    s = re.sub(
                        r'({POINTIDENTITY_LOG_DIR})',
                        lambda m: {'{POINTIDENTITY_LOG_DIR}': self.pointidentity_log_dir}[m.group()],
                        response.read().decode("utf-8").strip()
                    )
    ...
    def install(self) -> bool:
        ...
        try:
            # Download and extract pointidentity-node binary
            if self.get_binary():
                logging.info("Successfully downloaded and extracted pointidentity-noded binary")
            else:
                logging.error("Failed to download and extract binary")
                return False

            # Create pointidentity user if it doesn't exist
            if self.prepare_pointidentity_user():
                logging.info("User/group pointidentity setup successfully")
            else:
                logging.error("Failed to setup user/group pointidentity")
                return False
            ...
            # If Cosmovisor is not needed, treat it as a standalone installation
                ...
                if self.install_standalone():
                    logging.info("Successfully installed pointidentity-noded as a standalone binary")
                else:
                    logging.error("Failed to setup pointidentity-noded as a standalone binary")
                    return False

            # Setup pointidentity-noded environment variables
            # These are independent of Cosmovisor environment variables
            # Set them regardless of whether Cosmovisor is used or not
            self.set_pointidentity_env_vars()

            # Configure pointidentity-noded settings
            # This edits the config.toml and app.toml files
            if self.configure_node_settings():
                logging.info("Successfully configured pointidentity-noded settings")
            else:
                logging.error("Failed to configure pointidentity-noded settings")
                return False

            # Configure systemd service for pointidentity-noded
            ...
            logging.exception(f"Failed to install pointidentity-noded. Reason: {e}")

    def get_binary(self) -> bool:
        # Download pointidentity-noded binary and extract it
        # Also remove the downloaded archive file, if applicable
        try:
            logging.info("Downloading pointidentity-noded binary...")
            ...
            logging.exception(f"Failed to download pointidentity-noded binary. Reason: {e}")
            return False

    def prepare_pointidentity_user(self) -> bool:
        # Set pointidentity user/group along with required permissions
        try:
            # Create pointidentity user and group if they don't exist
            if not self.does_user_exist(DEFAULT_POINTIDENTITY_USER):
                logging.info(f"Creating {DEFAULT_POINTIDENTITY_USER} group")
                self.exec(f"addgroup {DEFAULT_POINTIDENTITY_USER} --quiet --system")

                logging.info(f"Creating {DEFAULT_POINTIDENTITY_USER} user and adding to {DEFAULT_POINTIDENTITY_USER} group")
                self.exec(
                    f"adduser --system {DEFAULT_POINTIDENTITY_USER} --home {self.pointidentity_home_dir} --shell /bin/bash --ingroup {DEFAULT_POINTIDENTITY_USER} --quiet")
            else:
                logging.debug(f"User {DEFAULT_POINTIDENTITY_USER} already exists. Skipping creation...")

            # Create ~/.node root directory
            if not os.path.exists(self.pointidentity_root_dir):
                logging.info(f"Creating root directory: {self.pointidentity_root_dir}")
                os.makedirs(self.pointidentity_root_dir, exist_ok=True)
            else:
                logging.info(f"Root directory {self.pointidentity_root_dir} already exists. Skipping creation...")

            # Set permissions for pointidentity home directory to pointidentity:pointidentity
            logging.info(f"Setting permissions for {self.pointidentity_home_dir} to {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER}")
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_home_dir}")

            # Return True if all steps were successful
            return True
        except Exception as e:
            logging.exception(f"Failed to create {DEFAULT_POINTIDENTITY_USER} user. Reason: {e}")
            return False
    ...
    def pre_install(self) -> bool:
        ...
        # 3. Remove existing pointidentity-noded data and configurations
        ...
        try:
            ...
            os.makedirs(self.pointidentity_backup_dir, exist_ok=True)
            ...
            if os.path.exists(self.pointidentity_config_dir):
                # Backup ~/.node/config/ folder
                shutil.copytree(self.pointidentity_config_dir, os.path.join(self.pointidentity_backup_dir, "config"), dirs_exist_ok=True)
                logging.info(f"Successfully backed up {self.pointidentity_config_dir} directory")
            else:
                logging.debug("No config folder found to backup. Skipping...")

            # Backup ~/.node/data/priv_validator_key.json
            # Without this file, a validator node will get jailed!
            if os.path.exists(os.path.join(self.pointidentity_data_dir, "priv_validator_state.json")):
                shutil.copy(os.path.join(self.pointidentity_data_dir, "priv_validator_state.json"),
                    os.path.join(self.pointidentity_backup_dir, "priv_validator_state.json"))
                logging.info(f"Successfully backed up {self.pointidentity_data_dir}/priv_validator_state.json")
            else:
                logging.debug("No validator state file found to backup. Skipping...")

            # Backup ~/.node/data/upgrade-info.json
            # This file is required for Cosmovisor to track and understand where upgrade is needed
            if os.path.exists(os.path.join(self.pointidentity_data_dir, "upgrade-info.json")):
                shutil.copy(os.path.join(self.pointidentity_data_dir, "upgrade-info.json"),
                    os.path.join(self.pointidentity_backup_dir, "upgrade-info.json"))
                logging.info(f"Successfully backed up {self.pointidentity_data_dir}/upgrade-info.json")
            else:
                logging.debug("No upgrade-info.json file found to backup. Skipping...")

            # Change ownership of backup directory to pointidentity user
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_backup_dir}")
            logging.debug(f"Successfully changed ownership of {self.pointidentity_backup_dir}")

            if self.interviewer.is_from_scratch:
                # Remove pointidentity-node data and binaries
                ...
                self.remove_safe(self.pointidentity_root_dir)
            else:
                logging.debug("No user data or configs to remove. Skipping...")

            # Setup logging related directories
            if not os.path.exists(self.pointidentity_log_dir):
                # Create ~/.node/log directory
                logging.info(f"Creating log directory: {self.pointidentity_log_dir}")
                os.makedirs(self.pointidentity_log_dir, exist_ok=True)

                # Create blank ~/.node/log/stdout.log file. Overwrite if it already exists.
                ...
                with open(os.path.join(self.pointidentity_log_dir, "stdout.log"), "w") as file:
                    logging.debug(f"Created blank stdout.log file in {self.pointidentity_log_dir}")
            else:
                logging.info(f"Skipping log directory creation because {self.pointidentity_log_dir} already exists")

            # Create symlink from pointidentity-noded log folder from /var/log/pointidentity-node
            # This step is necessary since many logging tools look for logs in /var/log
            if not os.path.exists("/var/log/pointidentity-node"):
                logging.info("Creating a symlink from pointidentity-noded log folder to /var/log/pointidentity-node")
                os.symlink(self.pointidentity_log_dir, "/var/log/pointidentity-node", target_is_directory=True)
            else:
                logging.debug("Skipping linking because /var/log/pointidentity-node already exists")
            ...
            # This file will be sourced by during the pointidentity user's LOGIN shells
            if not os.path.exists(self.pointidentity_user_bash_profile_path):
                logging.info(f"Creating {self.pointidentity_user_bash_profile_path} file")

                # Download the template file from GitHub and write it to ~/.bash_profile
                if is_valid_url(BASH_PROFILE_TEMPLATE):
                    with request.urlopen(BASH_PROFILE_TEMPLATE) as response, open(self.pointidentity_user_bash_profile_path, "w") as file:
                        # Add a shebang line
                ...
                # Change ownership to pointidentity:pointidentity
                shutil.chown(self.pointidentity_user_bash_profile_path, DEFAULT_POINTIDENTITY_USER, DEFAULT_POINTIDENTITY_USER)
            else:
                logging.debug(f"{self.pointidentity_user_bash_profile_path} already exists. Skipping creation...")

            # Create a ~/.bashrc file to set environment variables
            # This file will be sourced by the pointidentity user's NON-LOGIN shells
            if not os.path.exists(self.pointidentity_user_bashrc_path):
                logging.info(f"Creating {self.pointidentity_user_bashrc_path} file")

                # Create the file, overwrite if it already exists
                with open(self.pointidentity_user_bashrc_path, "w") as file:
                    # Add a shebang line
                    file.write("#!/bin/bash\n\n")

                # Change ownership to root:root
                shutil.chown(self.pointidentity_user_bashrc_path, DEFAULT_POINTIDENTITY_USER, DEFAULT_POINTIDENTITY_USER)
            else:
                logging.debug(f"{self.pointidentity_user_bashrc_path} already exists. Skipping creation...")

            # Change ownership of directories
            # Always execute these since they might be lost if directories are removed and recreated
            logging.info(f"Setting ownership of {self.pointidentity_home_dir} to {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER}")
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_home_dir}")

            logging.info(f"Setting ownership of {self.pointidentity_log_dir} to syslog:{DEFAULT_POINTIDENTITY_USER}")
            self.exec(f"chown -R syslog:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_log_dir}")
    ...
    def install_cosmovisor(self) -> bool:
        # Install binaries for pointidentity-noded and Cosmovisor
        # Cosmovisor is only installed if requested by the user
        # pointidentity-noded binary is installed in Cosmovisor bin path under this scenario
        try:
            ...
            # Move pointidentity-noded binary to /usr/bin
            logging.info(f"Copying pointidentity-noded binary from {self.temporary_node_binary_path} to {self.standalone_node_binary_path}")
            shutil.copy(self.temporary_node_binary_path, self.standalone_node_binary_path)

            # Set ownership of pointidentity-noded binary to root:root
            ...
            if not os.path.exists(self.cosmovisor_root_dir):
                self.exec(f"sudo -u {DEFAULT_POINTIDENTITY_USER} bash -c 'DAEMON_NAME={DEFAULT_BINARY_NAME} DAEMON_HOME={self.pointidentity_root_dir} cosmovisor init {self.standalone_node_binary_path}'")
            else:
                logging.info("Cosmovisor directory already exists. Skipping initialization...")

            # Remove pointidentity-noded binary from /usr/bin if it's not a symlink
            if not os.path.islink(self.standalone_node_binary_path):
                ...
                # Move pointidentity-noded binary to Cosmovisor bin path
                # shutil.move() will overwrite the file if it already exists
                logging.info(f"Moving pointidentity-noded binary from {self.temporary_node_binary_path} to {self.cosmovisor_current_bin_path}")
                shutil.move(self.temporary_node_binary_path, self.cosmovisor_current_bin_path)

                # Set ownership of pointidentity-noded binary to pointidentity:pointidentity
                # This is ONLY done when the binary is moved to Cosmovisor bin path
                shutil.chown(self.cosmovisor_current_bin_path, DEFAULT_POINTIDENTITY_USER, DEFAULT_POINTIDENTITY_USER)

                # Create symlink to pointidentity-noded binary in Cosmovisor bin path
            ...
            if self.interviewer.is_upgrade \
                and os.path.exists(os.path.join(self.pointidentity_data_dir, "upgrade-info.json")) \
                and not os.path.exists(os.path.join(self.cosmovisor_root_dir, "current/upgrade-info.json")):
                logging.info("Copying ~/.node/data/upgrade-info.json file to ~/.node/cosmovisor/current/")
            ...
            # Change owner of Cosmovisor directory to pointidentity:pointidentity
            logging.info(f"Changing ownership of {self.cosmovisor_root_dir} to {DEFAULT_POINTIDENTITY_USER} user")
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.cosmovisor_root_dir}")
    ...
    def install_standalone(self) -> bool:
        # Install pointidentity-noded as a standalone binary
        # pointidentity-noded binary is installed in /usr/bin under this scenario
        try:
            logging.info("Setting up standalone pointidentity-noded binary...")

            # Remove symlink for pointidentity-noded if it exists
            ...
            # Move pointidentity-noded binary to /usr/bin
            # shutil.move() will overwrite the file if it already exists
            logging.info(f"Moving pointidentity-noded binary from {self.temporary_node_binary_path} to {self.standalone_node_binary_path}")
    ...
    def set_cosmovisor_env_vars(self):
        ...
        try:
            ...
            self.set_environment_variable("DAEMON_HOME", self.pointidentity_root_dir)
    ...
    def set_pointidentity_env_vars(self):
        # Set environment variables for pointidentity-noded binary
        ...
        try:
            ...
            if self.interviewer.rpc_port:
                self.set_environment_variable("POINTIDENTITY_NODED_NODE", f"tcp://localhost:{self.interviewer.rpc_port}")
            else:
                # Otherwise, set to default value
                self.set_environment_variable("POINTIDENTITY_NODED_NODE", f"tcp://localhost:{DEFAULT_RPC_PORT}")

            # If chain ID is set, set to user-specified value
            if self.interviewer.chain == "testnet":
                self.set_environment_variable("POINTIDENTITY_NODED_CHAIN_ID", TESTNET_CHAIN_ID)
            elif self.interviewer.chain == "mainnet":
                self.set_environment_variable("POINTIDENTITY_NODED_CHAIN_ID", MAINNET_CHAIN_ID)
        except Exception as e:
            logging.exception(f"Failed to set environment variables for pointidentity-noded. Reason: {e}")
            raise
    ...
    def set_environment_variable(self, env_var_name, env_var_value):
        ...
        # 1. /etc/profile.d/pointidentity-noded.sh (for all users, LOGIN shell)
        # 2. ~/.bashrc (for current user, LOGIN shell)
        # 3. ~/.bash_profile (for current user, LOGIN shell)
        try:
            # Set environment variable in /etc/profile.d/pointidentity-noded.sh
            if os.path.exists(DEFAULT_LOGIN_SHELL_ENV_FILE_PATH):
                ...
                # Read /etc/profile.d/pointidentity-noded.sh line by line
                with open(DEFAULT_LOGIN_SHELL_ENV_FILE_PATH, "w") as file:
                    for line in lines:
            ...
            if os.path.exists(self.pointidentity_user_bashrc_path):
                with open(self.pointidentity_user_bashrc_path, "r") as file:
                    lines = file.readlines()
                ...
                with open(self.pointidentity_user_bashrc_path, "w") as file:
                    ...
                    # Add new value (if it doesn't exist in the file already)
                    if not updated:
                        logging.debug(f"Adding {env_var_name}={env_var_value} to {self.pointidentity_user_bashrc_path}")
                        file.write(f'export {env_var_name}={env_var_value}\n')
            else:
                logging.debug(f"{self.pointidentity_user_bashrc_path} doesn't exist. Skipped adding {env_var_name} to the file...")
    ...
    def configure_node_settings(self) -> bool:
        # Configure pointidentity-noded settings in app.toml and config.toml
        ...
        try:
            ...
            app_toml_path = os.path.join(self.pointidentity_config_dir, "app.toml")
            config_toml_path = os.path.join(self.pointidentity_config_dir, "config.toml")
            genesis_file_path = os.path.join(self.pointidentity_config_dir, 'genesis.json')
            ...
            if self.interviewer.is_from_scratch:
                ...
                if not os.path.exists(os.path.join(self.pointidentity_config_dir, 'priv_validator_key.json')):
                    # Initialize the node
                    logging.info(f"Initializing {self.pointidentity_root_dir} directory")
                    self.exec(f"sudo -u {DEFAULT_POINTIDENTITY_USER} bash -c 'pointidentity-noded init {self.interviewer.moniker}'")
                else:
                    logging.debug(f"Validator key already exists in {self.pointidentity_config_dir}. Skipping pointidentity-noded init...")
                ...
                logging.debug("Skipping pointidentity-noded init as setup is not needed")
            ...
            # Set ownership of configuration directory to pointidentity:pointidentity
            logging.info(f"Setting ownership of {self.pointidentity_config_dir} to {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER}")
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_config_dir}")

            # Return True if all the above steps were successful
            return True
        except Exception as e:
            logging.exception(f"Failed to configure pointidentity-noded settings. Reason: {e}")
            return False

    def setup_node_systemd(self) -> bool:
        # Setup pointidentity-noded related systemd services
        # If user selected Cosmovisor install, then pointidentity-cosmovisor.service will be setup
        # If user selected Standalone install, then pointidentity-noded.service will be setup
        ...
        try:
            # Remove pointidentity-noded.service and pointidentity-cosmovisor.service if they exist
            ...
            # Setup pointidentity-cosmovisor.service if requested
            if self.interviewer.is_cosmovisor_needed:
                # Write pointidentity-cosmovisor.service file
                ...
                # Enable pointidentity-cosmovisor.service
                self.enable_systemd_service(DEFAULT_COSMOVISOR_SERVICE_NAME)
                return True

            # Otherwise, setup pointidentity-noded.service for standalone install
            else:
                # Fetch the template file from GitHub
                if is_valid_url(STANDALONE_SERVICE_TEMPLATE):
                    ...
                    # Enable pointidentity-noded.service
            ...
            logging.exception(f"Failed to setup systemd service for pointidentity-node. Reason: {e}")
            return False

    # Setup logging related systemd services
    def setup_logging_systemd(self) -> bool:
        # Install pointidentity-node configuration for rsyslog if user wants to rewrite rsyslog service file
        # Also run if setup is from scratch/first-time install
        try:
            ...
            # Install pointidentity-node configuration for logrotate if user wants to rewrite logrotate service file
            # Also run if setup is from scratch/first-time install
            if self.interviewer.rewrite_logrotate:
                ...
                logging.info("Configuring logrotate systemd service for pointidentity-node logging")
    ...
    def download_snapshot(self) -> bool:
        ...
        try:
            ...
            if self.set_snapshot_url():
                ...
                file_path = os.path.join(self.pointidentity_root_dir, fname)
            else:
            ...
            if os.path.exists(self.pointidentity_backup_dir) and match:
                # Check that backup of validator keys, state, and upgrade info exists before proceeding
                logging.info(f"Backup directory exists: {self.pointidentity_backup_dir}.")

                # Remove contents of data directory
                logging.warning(f"Contents of {self.pointidentity_data_dir} will be deleted to make room for snapshot")
                self.remove_safe(self.pointidentity_data_dir)

                # Recreate data directory
                os.makedirs(self.pointidentity_data_dir, exist_ok=True)
                self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_data_dir}")
            else:
            ...
            # Check how much free disk space is available wherever the pointidentity home directory is located
            # First, determine where the home directory is mounted
            fs_stats = os.statvfs(self.pointidentity_home_dir)
            ...
            if (int(archive_size) < int(free_space)) and match:
                ...
                self.exec(f"wget -c {self.snapshot_url} -P {self.pointidentity_root_dir}")
    ...
    def set_snapshot_url(self) -> bool:
        # Get latest available snapshot URL from snapshots.pointidentity.net for the given chain
    ...
    def extract_snapshot(self):
        # Extract snapshot archive to pointidentity node data directory
        # This is a blocking operation that will take a while
        # Once extracted, restore files from backup folder
        try:
            # Set file path of snapshot archive
            file_path = os.path.join(self.pointidentity_root_dir, os.path.basename(self.snapshot_url))

            # Extract to pointidentity node data directory EXCEPT for validator state
            ...
            if os.path.exists(file_path):
                ...
                self.exec(f"sudo -u {DEFAULT_POINTIDENTITY_USER} bash -c 'pv {file_path} \
                    | tar --use-compress-program=lz4 -xf - -C {self.pointidentity_root_dir} \
            ...
            if os.path.exists(self.pointidentity_backup_dir):
                logging.info("Backup directory found. Restoring files from backup...")

                # Restore priv_validator_state.json
                if os.path.exists(os.path.join(self.pointidentity_backup_dir, "priv_validator_state.json")):
                    shutil.copy(os.path.join(self.pointidentity_backup_dir, "priv_validator_state.json"),
                        os.path.join(self.pointidentity_data_dir, "priv_validator_state.json"))
                    logging.info(f"Restored priv_validator_state.json to {self.pointidentity_data_dir}")
                else:
                    logging.warning(f"priv_validator_state.json not found in {self.pointidentity_backup_dir}! Please restore it manually to {self.pointidentity_data_dir}.")

                # Restore upgrade-info.json
                if os.path.exists(os.path.join(self.pointidentity_backup_dir, "upgrade-info.json")):
                    shutil.copy(os.path.join(self.pointidentity_backup_dir, "upgrade-info.json"),
                        os.path.join(self.pointidentity_data_dir, "upgrade-info.json"))
                    logging.info(f"Restored upgrade-info.json to {self.pointidentity_data_dir}")
                else:
                    logging.warning(f"upgrade-info.json not found in {self.pointidentity_backup_dir}! Please restore it manually to {self.pointidentity_data_dir}.")

                # If Cosmovisor is needed, copy upgrade-info.json to ~/.node/cosmovisor/current/ directory
                # Otherwise, Cosmovisor will throw an error
                if self.interviewer.is_cosmovisor_needed and os.path.exists(os.path.join(self.pointidentity_data_dir, "upgrade-info.json")):
                    shutil.copy(os.path.join(self.pointidentity_data_dir, "upgrade-info.json"),
                    ...
                    # Change ownership of Cosmovisor directory to pointidentity user
                    self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.cosmovisor_root_dir}")
                else:
                    logging.warning(f"upgrade-info.json not found in {self.pointidentity_data_dir}! Please restore it manually to {self.cosmovisor_root_dir}/current/")
            else:
                logging.warning(f"Backup folder not found. Please manually restore required files to {self.pointidentity_data_dir} and {self.pointidentity_config_dir}")

            # Change ownership of pointidentity node data directory to pointidentity user
            self.exec(f"chown -R {DEFAULT_POINTIDENTITY_USER}:{DEFAULT_POINTIDENTITY_USER} {self.pointidentity_data_dir}")
...
###############################################################
class Interviewer:
    def __init__(self, home_dir=DEFAULT_POINTIDENTITY_HOME_DIR):
        ...
        self._moniker = POINTIDENTITY_NODED_MONIKER
    ...
    @property
    def pointidentity_root_dir(self):
        return os.path.join(self.home_dir, ".node")
    ...
    # Check if pointidentity-noded is installed
    def is_node_installed(self) -> bool:
        try:
            return shutil.which("pointidentity-noded") is not None
        except Exception as e:
            logging.exception(f"Could not check if pointidentity-noded is already installed. Reason: {e}")
    ...
    # Get list of last N releases for pointidentity-node from GitHub
    def get_releases(self):
        try:
            req = request.Request(
                "https://api.github.com/repos/pointidentity/pointidentity-node/releases")
    ...
    def get_latest_release(self):
        try:
            req = request.Request(
                "https://api.github.com/repos/pointidentity/pointidentity-node/releases/latest")
            req.add_header("Accept", "application/vnd.github.v3+json")
            with request.urlopen(req) as response:
    ...
    # Ask user to select a version of pointidentity-node to install
    def ask_for_version(self):
        try:
            ...
            print(f"Latest stable pointidentity-noded release version is {default}")
            print("List of pointidentity-noded releases: ")
            ...
            release_num = int(self.ask(
                "Choose list option number above to select version of pointidentity-node to install", default=1))
            ...
            logging.exception(f"Failed to selected version of pointidentity-noded. Reason: {e}")

    # Set pointidentity user's home directory
    def ask_for_home_directory(self) -> str:
        try:
            self.home_dir = self.ask(
                "Set path for pointidentity user's home directory", default=DEFAULT_POINTIDENTITY_HOME_DIR)
            logging.debug(f"Setting home directory to {self.home_dir}")
        except Exception as e:
            logging.exception(f"Failed to set pointidentity user's home directory. Reason: {e}")
    ...
    def ask_for_chain(self):
        try:
            answer = int(self.ask(
                "Select pointidentity network to join:\n"
    ...
    def ask_for_cosmovisor(self):
        try:
            logging.info("Installing pointidentity-node with Cosmovisor allows for automatic unattended upgrades for valid software upgrade proposals. See https://docs.cosmos.network/main/tooling/cosmovisor for more information.\n")
            answer = self.ask(
                "Install pointidentity-noded using Cosmovisor? (yes/no)", default=DEFAULT_USE_COSMOVISOR)
            if answer.lower().startswith("y"):
    ...
    def ask_for_moniker(self):
        try:
            logging.info("Moniker is a human-readable name for your pointidentity-node.\nThis is NOT the same as your validator name, and is only used to uniquely identify your node for Tendermint P2P address book.\nIt can be edited later in your ~/.node/config/config.toml file.\n")
            self.moniker = self.ask(
                "Provide a moniker for your pointidentity-node", default=POINTIDENTITY_NODED_MONIKER)
            if self.moniker is not None and isinstance(self.moniker, str):
                logging.debug(f"Moniker set to {self.moniker}")
            else:
                logging.error("Invalid moniker provided during pointidentity-noded setup.\n")
    ...
    def ask_for_gas_price(self):
        try:
            logging.info(
                "Minimum gas prices is the price you are willing to accept as a validator to process a transaction.\nValues should be entered in format <number>upoint (e.g., 50upoint)\n")
            self.gas_price = self.ask("Specify minimum gas price", default=POINTIDENTITY_NODED_MINIMUM_GAS_PRICES)
            if self.gas_price.endswith("upoint"):
                logging.debug(f"Minimum gas price set to {self.gas_price}")
            else:
                logging.error("Invalid minimum gas price provided. Valid format is <number>upoint.\n")
    ...
    def ask_for_log_level(self):
        try:
            self.log_level = self.ask(
                "Specify log level (trace|debug|info|warn|error|fatal|panic)", default=POINTIDENTITY_NODED_LOG_LEVEL)
    ...
    def ask_for_log_format(self):
        try:
            self.log_format = self.ask("Specify log format (json|plain)", default=POINTIDENTITY_NODED_LOG_FORMAT)
    ...
    def ask_for_upgrade(self):
        try:
            logging.warning("Existing pointidentity-node binary detected.\n")
    ...
    def ask_for_rewrite_rsyslog(self):
        try:
            answer = self.ask("Overwrite existing configuration for pointidentity-node logging? (yes/no)", default="yes")
            if answer.lower().startswith("y"):
...
if __name__ == '__main__':
    ...
    # 1. Version of pointidentity-noded to install
    # 2. Home directory for pointidentity user
    # 3. Install new version of pointidentity-noded
    # 4. Chain ID to join
    ...
    try:
        ...
        # Check if pointidentity-noded is already installed
        ...
        # If no pointidentity-noded binary is found, install from scratch
        if installed is False:
            install_steps()

        else:
            # If pointidentity-noded binary is found, ask user if they want to upgrade or install from scratch
    ...
    try:
        ...
        if installer.install():
            logging.info(f"Installation of pointidentity-noded {installer.version} completed successfully!\n")
            logging.info("Please review the configuration files manually and use systemctl to start the node.\n")
            logging.info("Documentation: https://docs.pointidentity.com/node\n")
            sys.exit(0)
        else:
            logging.error(f"Installation of pointidentity-noded {installer.version} failed. Exiting...")
            logging.info("Documentation: https://docs.pointidentity.com/node\n")
            sys.exit(1)
