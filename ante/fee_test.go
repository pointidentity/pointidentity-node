...
import (
	...
	"cosmossdk.io/math"
	pidante "github.com/pointidentity/pointidentity-node/ante"
	pidpost "github.com/pointidentity/pointidentity-node/post"
...
var _ = Describe("Fee tests on CheckTx", func() {
	...
	It("Ensure Zero Mempool Fees On Simulation", func() {
		mfd := pidante.NewOverAllDecorator(decorators...)
	...
	It("Ensure Mempool Fees", func() {
		...
		price := sdk.NewDecCoinFromDec(didtypes.BaseMinimalDenom, sdk.NewDec(200_000_000_000))
		...
		price = sdk.NewDecCoinFromDec(didtypes.BaseMinimalDenom, sdk.NewDec(0).Quo(sdk.NewDec(didtypes.BaseFactor)))
		lowGasPrice := []sdk.DecCoin{price}
		s.ctx = s.ctx.WithMinGasPrices(lowGasPrice) // 1 upoint
		...
		// of 10000000000upoint, the priority here is 10*10^9.
	...
	It("TaxableTx Mempool Inclusion", func() {
		...
		dfd := pidante.NewOverAllDecorator(decorators...)
...
var _ = Describe("Fee tests on DeliverTx", func() {
	...
	It("TaxableTx Lifecycle", func() {
		...
		taxDecorator := pidpost.NewTaxDecorator(s.app.AccountKeeper, s.app.BankKeeper, s.app.FeeGrantKeeper, s.app.DidKeeper, s.app.ResourceKeeper, s.app.FeeMarketKeeper)
		...
		burnt := pidante.GetBurnFeePortion(feeParams.BurnFactor, sdk.NewCoins(feeParams.CreateDid))
		...
		reward := pidante.GetRewardPortion(sdk.NewCoins(feeParams.CreateDid), burnt)
...
var _ = Describe("Test Deduct Coins", func() {
	...
	It("valid coins", func() {
		...
		err = pidpost.DeductCoins(s.app.BankKeeper, s.ctx, coins, false)
	...
	It("valid zero coin", func() {
		...
		err = pidpost.DeductCoins(s.app.BankKeeper, s.ctx, feeCoin, false)
...
var _ = Describe("Test Deduct coins and distribute fees", func() {
	...
	It("valid coins and distribute to feeCollector", func() {
		...
		err = pidpost.DeductCoins(s.app.BankKeeper, s.ctx, deductFee, distributeFees)
		Expect(err).To(BeNil(), "Error on deducting coins")
...
var _ = Describe("Test Send tip", func() {
	...
	It("valid coins", func() {
		...
		err = pidpost.SendTip(s.app.BankKeeper, s.ctx, proposer, tip)
...
var _ = Describe("Test PostHandle", func() {
	...
	It("signer has enough funds, should pass with tip", func() {
		...
		taxDecorator := pidpost.NewTaxDecorator(
			s.app.AccountKeeper,
			s.app.BankKeeper,
			s.app.FeeGrantKeeper,
			s.app.DidKeeper,
