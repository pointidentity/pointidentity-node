...
import (
	...
	pidapp "github.com/pointidentity/pointidentity-node/app"
	didtypes "github.com/pointidentity/pointidentity-pointidentity/x/did/types"
...
func createTestApp(isCheckTx bool) (*pidapp.TestApp, sdk.Context, error) {
	app, err := pidapp.Setup(isCheckTx)
	if err != nil {
...
func (s *AnteTestSuite) SetupTest(isCheckTx bool) error {
	...
	// Set up TxConfig.
	encodingConfig := pidapp.MakeTestEncodingConfig()
	...
	anteHandler, err := pidapp.NewAnteHandler(
		pidapp.HandlerOptions{
...
func (s *AnteTestSuite) CreateTestAccounts(numAccs int) ([]TestAccount, error) {
	...
	for i := 0; i < numAccs; i++ {
		...
		someCoins := sdk.Coins{
			sdk.NewInt64Coin(didtypes.BaseMinimalDenom, 1000000*1e9), // 1mn POINT
		}
