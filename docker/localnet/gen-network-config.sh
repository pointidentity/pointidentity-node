...
# Params
CHAIN_ID=${1:-"pointidentity"} # First parameter, default is "pointidentity"
...
function init_node() {
  ...
  pointidentity-noded init "${NODE_MONIKER}" --chain-id "${CHAIN_ID}" --home "${NODE_HOME}" 2> /dev/null
  pointidentity-noded tendermint show-node-id --home "${NODE_HOME}" > "${NODE_HOME}/node_id.txt"
  pointidentity-noded tendermint show-validator --home "${NODE_HOME}" > "${NODE_HOME}/node_val_pubkey.txt"
}
...
function configure_node() {
  ...
  sed -i $SED_EXT 's/minimum-gas-prices = ""/minimum-gas-prices = "50upoint"/g' "${APP_TOML}"
  sed -i $SED_EXT 's/enable = false/enable = true/g' "${APP_TOML}"
  sed -i $SED_EXT 's|laddr = "tcp://127.0.0.1:26647"|laddr = "tcp://0.0.0.0:26647"|g' "${CONFIG_TOML}"
...
function configure_genesis() {
  ...
  # Test accounts
  BASE_ACCOUNT_1="pointid1rnr5jrt4exl0samwj0yegv99jeskl0hsxmcz96"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_1}'", "coins": [{"denom": "upoint", "amount": "100001000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP" && \
  ...
  BASE_ACCOUNT_2="pointid1l9sq0se0jd3vklyrrtjchx4ua47awug5vsyeeh"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_2}'", "coins": [{"denom": "upoint", "amount": "100001000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP"  && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  BASE_ACCOUNT_3="pointid14wymr6h8a0rvynwayytssy5rhn3evltcpxh0zh"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_3}'", "coins": [{"denom": "upoint", "amount": "100"}] }]' "$GENESIS" > "$GENESIS_TMP"  && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  BASE_ACCOUNT_4="pointid1c8tq9wcfwtunkunng5pe442jlmwns9sgve42mf"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_4}'", "coins": [{"denom": "upoint", "amount": "100001000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP"  && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  BASE_ACCOUNT_5="pointid1xnx6fe4fg4etlaxm0ty25j3ae5thhadraskplv"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_5}'", "coins": [{"denom": "upoint", "amount": "100001000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP"  && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  BASE_ACCOUNT_6="pointid1makysw7c788c7fzkfzn7al80wyfl4at6jtjyg4"
  ...
  jq '.app_state.bank.balances += [{"address": "'${BASE_ACCOUNT_6}'", "coins": [{"denom": "upoint", "amount": "100"}] }]' "$GENESIS" > "$GENESIS_TMP"  && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  BASE_VESTING_ACCOUNT="pointid1lkqddnapqvz2hujx2trpj7xj6c9hmuq7uhl0md"
  ...
  BASE_VESTING_COIN="{\"denom\":\"upoint\",\"amount\":\"10001000000000000\"}"
  jq '.app_state.bank.balances += [{"address": "'${BASE_VESTING_ACCOUNT}'", "coins": [{"denom": "upoint", "amount": "5000000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP" && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  CONTINUOUS_VESTING_ACCOUNT="pointid1353p46macvn444rupg2jstmx3tmz657yt9gl4l"
  ...
  jq '.app_state.bank.balances += [{"address": "'${CONTINUOUS_VESTING_ACCOUNT}'", "coins": [{"denom": "upoint", "amount": "5000000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP" && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  DELAYED_VESTING_ACCOUNT="pointid1njwu33lek5jt4kzlmljkp366ny4qpqusahpyrj"
  ...
  jq '.app_state.bank.balances += [{"address": "'${DELAYED_VESTING_ACCOUNT}'", "coins": [{"denom": "upoint", "amount": "5000000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP" && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  PERIODIC_VESTING_ACCOUNT="pointid1uyngr0l3xtyj07js9sdew9mk50tqeq8lghhcfr"
  ...
  jq '.app_state.bank.balances += [{"address": "'${PERIODIC_VESTING_ACCOUNT}'", "coins": [{"denom": "upoint", "amount": "5000000000000000"}] }]' "$GENESIS" > "$GENESIS_TMP" && \
    mv "${GENESIS_TMP}" "${GENESIS}"
  ...
  jq '.app_state.bank.supply += [{"denom": "upoint", "amount": "420004000000000200"}]' "$GENESIS" > "$GENESIS_TMP" && mv "${GENESIS_TMP}" "${GENESIS}"
}
...
do
  ...
  echo "${mnemonics[$i]}" | pointidentity-noded keys add "operator-$i" --keyring-backend "test" --home "${NODE_HOME}" --recover
  pointidentity-noded keys list --keyring-backend "test" --home "${NODE_HOME}"
  pointidentity-noded genesis add-genesis-account "operator-$i" 20000000000000000upoint --keyring-backend "test" --home "${NODE_HOME}"

  NODE_ID=$(pointidentity-noded tendermint show-node-id --home "${NODE_HOME}")
  NODE_VAL_PUBKEY=$(pointidentity-noded tendermint show-validator --home "${NODE_HOME}")
  pointidentity-noded genesis gentx "operator-$i" 1000000000000000upoint --chain-id "${CHAIN_ID}" --node-id "${NODE_ID}" \
...
pointidentity-noded genesis collect-gentxs --home "${TMP_NODE_HOME}"
pointidentity-noded genesis validate-genesis --home "${TMP_NODE_HOME}"
