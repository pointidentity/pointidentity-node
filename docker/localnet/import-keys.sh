...
function import_key() {
    ...
    if pointidentity-noded keys show "${ALIAS}" --keyring-backend ${KEYRING_BACKEND}
    then
      echo "Key ${ALIAS} already exists"
      return 0
    fi

    echo "${MNEMONIC}" | pointidentity-noded keys add "${ALIAS}" --keyring-backend ${KEYRING_BACKEND} --recover
}
