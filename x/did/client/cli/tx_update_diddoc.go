...
Example payload file:
{
    "payload": {
        ...
        "id": "did:pointid:<namespace>:<unique-identifier>",
        "controller": [
            "did:pointid:<namespace>:<unique-identifier>"
        ],
        "authentication": [
            "did:pointid:<namespace>:<unique-identifier>#<key-id>"
        ],
        "assertionMethod": [],
        ...
        "verificationMethod": [
            {
                "id": "did:pointid:<namespace>:<unique-identifier>#<key-id>",
                "type": "<verification-method-type>",
                "controller": "did:pointid:<namespace>:<unique-identifier>",
                "publicKeyMultibase": "<public-key>"
            }
        ],
        "service": [
			{
                "id": "did:pointid:<namespace>:<unique-identifier>#<service-id>",
                "type": "<service-type>",
                "serviceEndpoint": [
                    "<service-endpoint>"
                ]
	...
	"signInputs": [
        {
            "verificationMethodId": "did:pointid:<namespace>:<unique-identifier>#<key-id>",
	...
	cmd.Flags().String(flags.FlagFees, sdk.NewCoin(types.BaseMinimalDenom, sdk.NewInt(types.DefaultUpdateDidTxFee)).String(), "Fixed fee for DID update, e.g., 25000000000upoint. Please check what the current fees by running 'pointidentity-noded query params subspace pointidentity feeparams'")
